...
#ifdef MACOS_X
// GLFW Cocoa forbids glfwPollEvents off the main thread. The frontend thread
// would call it from GetDirectUsercmd and AppKit would abort.
idCVar com_smp( "com_smp", "0", CVAR_RENDERER | CVAR_BOOL | CVAR_ARCHIVE, "run game modeling and renderer frontend in second thread, parallel to renderer backend" );
#else
